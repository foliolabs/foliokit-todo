...
if ( !class_exists('Puc_Todo_StateStore', false) ):

	class Puc_Todo_StateStore {
		/**
		...
		/**
		 * @var Puc_Todo_Update|null Cached update.
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(Puc_Todo_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( Puc_Todo_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(Puc_Todo_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = Puc_Todo_Utils::get($state, 'checkedVersion', '');
			$this->update = null;

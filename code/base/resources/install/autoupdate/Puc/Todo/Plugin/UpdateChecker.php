<?php
if ( !class_exists('Puc_Todo_Plugin_UpdateChecker', false) ):
	 ...
	 * @access public
	 */
	class Puc_Todo_Plugin_UpdateChecker extends Puc_Todo_UpdateChecker {
	    const URL_PREFIX = 'foliolabs=1&plugin=';
		...
		public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = ''){
            ...
            $this->addQueryArgFilter(function($args) {
                global $wp_version;

                $args['php_version'] = phpversion();
                $args['wordpress_version'] = $wp_version;
        ...
        public function getApiKey() {
		    $api_key_file = dirname($this->pluginAbsolutePath).'/base/resources/install/.api.key';

		    if (is_file($api_key_file)) {
                $file = trim(@file_get_contents($api_key_file));
                ...
                if (!$filterAdded && strpos($package, $needle) !== false) {
                    add_filter('http_request_args', function($requestArgs, $url = '') use($needle) {
                        ...
                        if (strpos($url, $needle) !== false) {
                            ...
                            if ($api_key = $this->getApiKey()) {
                                $requestArgs['headers']['Authorization'] = 'Bearer '.$this->getApiKey();
                            }
		 ...
		 * @return Puc_Todo_Scheduler
		 */
		protected function createScheduler($checkPeriod) {
			$scheduler = new Puc_Todo_Scheduler($this, $checkPeriod, array('load-plugins.php'));
		 ...
		 * @return Puc_Todo_Plugin_Info
		 */
		public function requestInfo($queryArgs = array()) {
			list($pluginInfo, $result) = $this->requestMetadata('Puc_Todo_Plugin_Info', 'request_info', $queryArgs);

			if ( $pluginInfo !== null ) {
				/** @var Puc_Todo_Plugin_Info $pluginInfo */
		 ...
		 *
		 * @return Puc_Todo_Update|null An instance of Plugin_Update, or NULL when no updates are available.
		 */
		public function requestUpdate() {
			...
			$update = Puc_Todo_Plugin_Update::fromPluginInfo($pluginInfo);
		 ...
		 *
		 * @return Puc_Todo_Plugin_Update|null
		 */
		public function getUpdate() {
			$update = parent::getUpdate();
			if ( isset($update) ) {
				/** @var Puc_Todo_Plugin_Update $update */
		...
		protected function createDebugBarExtension() {
			return new Puc_Todo_DebugBar_PluginExtension($this);
		}
	}
